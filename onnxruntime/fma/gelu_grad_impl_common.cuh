...
__device__ __inline__ T ComputeGeluGradScalar(T dY, T X, gelu_computation_mode::Approximation) {
  ...
  const float x_sqrt_param_mul = sqrt_param * X_float;
  float x2mul = X_float * X_float * mul_param;
  // float tan_h = tanhf(x_sqrt_param_mul * (1 + x2mul));
  float tan_h = tanhf(_Fma(x_sqrt_param_mul, x2mul, x_sqrt_param_mul));
  // float dg1 = 0.5f * (1.0f + tan_h);
  float dg1 = _Fma(0.5f, tan_h, 0.5f);
  float dg2 = x_sqrt_param_mul * 0.5f * (1 - tan_h * tan_h);

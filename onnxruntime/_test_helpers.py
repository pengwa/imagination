...
def run_with_pytorch_on_device(device, model, input_list, label_input, is_eval_mode=False):
    ...
    # warm up
    for i in range(20):
        with nvtx.annotate("pt_one_loop_warmup", color="yellow"):
    ...
    with torch.autograd.profiler.profile(profile_memory=True, use_cuda=True) as prof:
        for i in range(121):
            ...
            with nvtx.annotate("pt_one_loop", color="yellow"):
                output = model(*inputs_on_device)
                ...
                report_memory("after iteration: " + str(i))
                    # for name, param in model.named_parameters():
                    #     if param.requires_grad:
                    #         grad_outputs.append(param.grad)
    print("pt.torch.autograd.profiler.profile: ", prof.key_averages().table())
    return forward_outputs, grad_outputs, start, end
...
def run_with_ort_on_device(device, model, input_list, label_input, is_eval_mode=False):
    ...
    for i in range(20):
        with nvtx.annotate("ort_one_loop_warmup", color="yellow"):
            output = model(*inputs_on_device)
            forward_outputs = [output]
            grad_outputs = []
    ...
    with torch.autograd.profiler.profile(profile_memory=True, use_cuda=True) as prof:
        for i in range(121):
            ...
            with nvtx.annotate("ort_one_loop", color="yellow"):
                output = model(*inputs_on_device)
                forward_outputs = [output]
                grad_outputs = []
    ...
    print("ort.torch.autograd.profiler.profile: ", prof.key_averages().table())
    return forward_outputs, grad_outputs, start, end

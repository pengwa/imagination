...
regexp="([0-9.a-zA-Z_\-:\/ ]+) ([a-z0-9]+)_g([0-9]+)_phase1b([0-9]+)_phase2b([0-9]+): training throughput phase([0-9]+): ([0-9]+.*[0-9]*) sequences\/second"
  ...
  for line in f:
    ...
    if match:
      if match.group(6) == "1":
        ...
        batch=match.group(5)
      key=match.group(2) + "_g" + str(match.group(3)).zfill(2) + "_phase" + match.group(6) + "_b" + str(batch).zfill(2)
      
      #key = str(match.group(1)) + match.group(3)
      throuput = match.group(7)
      if key not in sums:
        sums[key] = 0.0
        freqs[key] = 0
...
print("('fptype_gpunum_batch', averaged throughput , collected_run_cnt)")
for k in sorted(res):
